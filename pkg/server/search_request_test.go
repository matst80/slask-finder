...
func TestParseQueryValues(t *testing.T) {
	...
	// }
	// if sr.StringFilter[1].Id != 11 || sr.StringFilter[1].Value != "elva" {
	// 	t.Errorf("Expected string filter to be [11:elva], got %v", sr.StringFilter)
	// }
	// if sr.NumberFilter[0].Id != 12 || sr.NumberFilter[0].Min != 12 || sr.NumberFilter[0].Max != 12 {
	// 	t.Errorf("Expected number filter to be [12:12], got %v", sr.NumberFilter)
	// }
}
